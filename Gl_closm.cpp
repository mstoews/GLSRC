...
void TGLCloseMonth::Ok (RTMessage)
{
 ...
 while (!glbtch->eof())
 {
  ...
  {
	 MessageBox(HWindow,GetString(GetApplication()->hInstance,8114),((TAcctModule *)GetApplication())->GetAppName(),MB_ICONINFORMATION);
	 cb.closeAll();
	 cb.init();
    ShutDownWindow();
	 return;
...
void TGLCloseMonth::CreateDistBackup(void)
{
 ...
 if(!gldistb->isValid())
  {
	ErrorClose(gldistb->GetDbfName());
	return;
  }

 Relate4set fQuery(gldist);
 fQuery.querySet(".NOT. DELETED()");
