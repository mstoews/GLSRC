...
public:
   ...
   int    pack()                    { return d4pack( data ) ; }
	...
	int    UpdateUserTime(char* szUser);
   #ifdef __TURBOC__
   ...
   void   select( Tag4 tag ) ;
	int    SetIndex(char *tagName,Code4* cb);
	void   SetIndex(string strTagName)   { d4tag_select ( data,d4tag(data,(LPSTR)strTagName.c_str()) ) ; }
...
public:
   ...
   char& operator[]( int index ) ;
	...
	void assign(double dAmount){ assignDouble(dAmount);}
	void assignLong( long, int new_len = -1, int zeros_in_front = 0 ) ;
	void assign(long lAmount){assignLong(lAmount);}
...
public:
	...
	int  operator= (double d){assignDouble(d);return TRUE;}
	int  operator= (string& s){assign((LPSTR)s.c_str());return TRUE;}
	int  operator= (string* s){assign((LPSTR)s->c_str());return TRUE;}
