...
#define   ID_CLEARED		   2001
#define   ID_AR_ACCTS       	1013
...
class TAPCashAccts : public TMainDialog
{
 ...
 virtual void Clear  (RTMessage Msg) = [CM_FIRST + ID_CLEARED];
...
void TAPCashAccts::Ok(RTMessage Msg)
{
 ...
 else if(MessageBox(HWindow,GetString(6022),((TAcctModule *)GetApplication())->GetAppName(),MB_ICONQUESTION|MB_YESNO) ==IDYES)
 {
  Modify(Msg);
  Clear(Msg);
 }
...
void TAPCashAccts::Modify(RTMessage)
{
 ...
 gl_setat.type.assign(szType);
}

void TAPCashAccts::Delete(RTMessage)
{
 if(MessageBox(HWindow,GetString(6024),((TAcctModule *)GetApplication())->GetAppName(),MB_ICONQUESTION|MB_YESNO) ==IDYES)
  {
	gl_setat.deleteRec();
	gl_setat.pack();
	Redraw();
...
void TGLSetPeriodDates::ResetDepreciation (RTMessage)
{
  ...
  if(MessageBox(HWindow,GetString(6025),((TAcctModule *)GetApplication())->GetAppName(),MB_ICONINFORMATION | MB_YESNO) == IDYES)
  {
...
void TGLProfile::GetCalc(RTMessage)
{
 ...
 if(MessageBox(HWindow,GetString(6026),((TAcctModule *)GetApplication())->GetAppName(),MB_ICONQUESTION | MB_YESNO) == IDYES)
  {
	if(MessageBox(HWindow,GetString(6026),((TAcctModule *)GetApplication())->GetAppName(),MB_ICONQUESTION | MB_YESNO) == IDYES)
	{
 ...
 {
  GetWindowsDirectory((LPSTR) szMsg, sizeof(szMsg));
  string  glini;
  glini = "notepad ";
  glini += szMsg;
  glini += "\\gl.ini";
  WinExec((LPSTR)glini.c_str(),SW_SHOW);
 }
...
void TGLProfile::Ok  (RTMessage)
{
 ...
 strcpy(CurDir,StrTrim(GetField(ID_DIRECTORY)));

 WritePrivateProfileString("GL_DIR","COMPANY",GetField(ID_VENDORNO),"GL.INI");
 WritePrivateProfileString("GL_DIR","DIR",CurDir,"GL.INI");
 _chdrive(drive);
 if(chdir(GetField(ID_DIRECTORY)) != 0)
  {
