...
if(glTag.open(cb,"GL_TAG"))
  {
   ...
   for(glTag.top();!glTag.eof();glTag.skip())
    {
      MessageBox(GetFocus(),glTag.alias(),fTagName.str(),MB_OK);
      Tag4 tag;
      for(tag.initFirst(db);tag.isValid();tag.initNext())
	{
	 if(strcmp(StrTrim(tag.alias()),StrTrim(fTagName.str())) == 0)
	   bTest = FALSE;
	  ...
	  char szMsg[200];
          wsprintf(szMsg,"File name = %s, Tag Name = %s, Tag Alias = %s",glTag.alias(),tag.alias(),fTag.str());
