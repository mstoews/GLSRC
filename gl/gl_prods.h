...
_CLASSDEF(TGLProd)
class TGLProd : public TMainDialog
{
 public: 
 PTprods glprod;  // Products database
 PTGLProdList PRODUCTS;
 BOOL bProds;

 TGLProd(PTWindowsObject AParent, LPSTR AName, PTModule AModule = NULL): TMainDialog (AParent,AName){};
 TGLProd(PTWindowsObject AParent, int ResourceId, PTModule AModule = NULL): TMainDialog (AParent,ResourceId){};
 ...
 virtual void Current  (RTMessage Msg) = [CM_FIRST + CM_UPDATE]{SetData(Msg);}
