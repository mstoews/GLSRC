...
if (apdist->seek(lGetField(ID_REQUESTNO)) == 0)
 {
  if(MessageBox(HWindow,"Do you wish to delete this request?",((TAcctModule *)GetApplication())->GetAppName(),MB_ICONQUESTION | MB_YESNO) == IDYES)
  {
...
void TAPRequestDetail::HandleListBox(RTMessage Msg)
{
 ...
 MessageBox(HWindow,GetString(6112),((TAcctModule *)GetApplication())->GetAppName(),MB_OK);
 delete [] szInvoiceno;
 delete [] szInvoice;
}
...
void TAPClearRequest::List(RTMessage)
{
 if(bRequestList)
 {
	 ...
	 // "PAYMENTCTL = 'P'"
	 rQuery.querySet(GetString(6109));
    Str4large szRequest;
    int i = 1;
    double compare = 0;
    double request = 0;
